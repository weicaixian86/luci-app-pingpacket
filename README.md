...
- OpenWrt `24.10.0` `x86/64`，发布 `ipk`
- OpenWrt `25.12.0` `x86/64`，发布 `apk`

触发方式：

- 在 GitHub Actions 中手动运行 `workflow_dispatch`
- 输入 `Release tag`，例如 `v1.0.22`

发布文件命名规则：

- 当 `Release tag=v1.0.22` 时，发布文件名为 `luci-app-pingpacket_v1.0.22_all.ipk` 和 `luci-app-pingpacket_v1.0.22_all.apk`
- 当 `Release tag=v1.0.23` 时，发布文件名为 `luci-app-pingpacket_v1.0.23_all.ipk` 和 `luci-app-pingpacket_v1.0.23_all.apk`
